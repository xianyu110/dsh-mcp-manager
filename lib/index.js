 ...
 *   - Two auth modes per server: OAuth (browser redirect, RFC 6749 + PKCE,
 *     RFC 7591 dynamic client registration) or a static Bearer token
 *   - Connects over MCP Streamable HTTP, registers each server's tools into
 *     `ctx.tools` as `mcp__<serverName>__<rawName>` (same convention as the
 *     built-in @deepseek-ai/dsh-mcp-client)
 ...
 *   POST /mcp-manager/api/servers         add a server {name,url,authMode,token?}
...
export function apply(ctx) {
  ...
  function accessToken(server) {
    if (server.authMode === 'static') return server.staticToken ?? '';
  ...
  async function mcpRpc(server, method, params, conn, { isNotification = false } = {}) {
    ...
    const buildHeaders = () => {
      const headers = { Authorization: `Bearer ${accessToken(server)}` };
      if (conn.sessionId) headers['Mcp-Session-Id'] = conn.sessionId;
      return headers;
    };
    let resp = await httpPostJson(server.url, buildHeaders(), payload);
    if (resp.status === 401 && server.authMode === 'oauth' && (await refreshTokens(server))) {
      resp = await httpPostJson(server.url, buildHeaders(), payload);
    }
  ...
  async function connect(server) {
    ...
    try {
      ...
      let resp = await httpPostJson(server.url, { Authorization: `Bearer ${accessToken(server)}` }, initPayload);
      if (resp.status === 401 && server.authMode === 'oauth' && (await refreshTokens(server))) {
        resp = await httpPostJson(server.url, { Authorization: `Bearer ${accessToken(server)}` }, initPayload);
      }
      ...
      // swap generation: dispose the previous tools, register the new set
      ...
      for (const tool of tools) {
        ...
        const definition = {
          ...
          async execute(args) {
            const r = await mcpRpc(server, 'tools/call', { name: tool.name, arguments: args ?? {} }, conn);
      ...
      conn.tools = new Map();
      conn.toolCount = 0;
      conn.status = server.authMode === 'oauth' && !server.oauth?.tokens ? 'needs-auth' : 'error';
  ...
  function serverView(server) {
    ...
    return {
      ...
      status: conn?.status ?? (server.authMode === 'oauth' && !server.oauth?.tokens ? 'needs-auth' : 'disconnected'),
      ...
      try {
        ...
        if (req.method === 'POST' && rest === '/servers') {
          ...
          const server = {
            ...
            name, url: serverUrl, authMode,
            ...(authMode === 'static' ? { staticToken: String(body.token ?? '') } : {}),
          };
          state.servers.push(server);
          saveState(state);
          if (server.authMode === 'static' && server.staticToken) connect(server);
          else setLive(server.id, { status: 'needs-auth', error: server.authMode === 'static' ? 'missing token' : '' });
        ...
        if (idMatch) {
          ...
          if (req.method === 'POST' && action === '/connect') {
            if (server.authMode === 'static' && !server.staticToken) {
              const body = await readBody(req);
              if (typeof body.token === 'string' && body.token) { server.staticToken = body.token; saveState(state); }
            }
            const conn = await connect(server);
  ...
  // ---------- startup: auto-connect servers that have credentials ----------
  for (const server of state.servers) {
    if (server.authMode === 'static' ? server.staticToken : server.oauth?.tokens) connect(server);
