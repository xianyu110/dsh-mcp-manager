...
export function apply(ctx) {
    ...
    async handler(req, res) {
      ...
      try {
        ...
        if (req.method === 'POST' && rest === '/servers') {
          ...
          else if (server.authMode === 'static' && server.staticToken) connect(server);
        ...
        if (idMatch) {
          ...
          if (req.method === 'POST' && action === '/enabled') {
            ...
            } else if (server.authMode === 'static' ? server.staticToken : server.oauth?.tokens) {
              await connect(server);
            } else {
              setLive(server.id, { status: 'needs-auth', error: server.authMode === 'static' ? 'missing token' : '' });
            }
            return json(res, 200, { server: serverView(server) });
          }
  ...
  for (const server of state.servers) {
    ...
    else if (server.authMode === 'static' ? server.staticToken : server.oauth?.tokens) connect(server);
