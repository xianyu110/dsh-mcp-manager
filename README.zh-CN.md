...
- **stdio 本地进程**：直接跑 `npx` / `uvx` / `python` 等命令，插件用 JSON-RPC over stdin/stdout 与之通信（自动拉起子进程、重连、退出时回收），无需任何远程服务器或认证。
...
2. **＋ 添加 MCP 服务器**：
   - **HTTP**：名称（决定 `mcp__<name>__*` 前缀）、URL、认证方式（OAuth 或静态 token）。
   - **stdio**：名称、命令（如 `npx`）、参数（空格分隔，可用引号包住含空格的参数）、可选的环境变量（JSON 对象）与工作目录。
...
- stdio 服务器以子进程常驻运行，随插件生命周期存活；`args` 按空格分词（引号可保护含空格的参数），不含 shell 展开（`~`、`$VAR`、管道等需自行写绝对路径或环境变量）。
